...
function requestCurrentServerTime(callback: (response: ServerTimeResponese) => void) {
    callback({
        status: 'success',
        data: Date.now()
    });
...
function requestCoffeeMachineQueueLength(callback: (response: CoffeeApiResponse) => void) {
    callback({
        status: 'error',
